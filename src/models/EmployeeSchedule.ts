...
EmployeeScheduleSchema.index({ salonId: 1, employeeId: 1, locationId: 1, status: 1 });
EmployeeScheduleSchema.index({ employeeId: 1, effectiveFrom: 1, effectiveUntil: 1 });
...
EmployeeScheduleSchema.pre("validate", function () {
  ...
  this.days.forEach((day, index) => {
    ...
    if (!intervalsAreValid(day.shifts)) {
      this.invalidate(`days.${index}.shifts`, "Shift intervals must be valid and non-overlapping");
    }
    if (!intervalsAreValid(day.breaks)) {
      this.invalidate(`days.${index}.breaks`, "Break intervals must be valid and non-overlapping");
    }
    ...
    if (!breaksAreInsideShifts) {
      this.invalidate(`days.${index}.breaks`, "Every break must be fully inside a shift");
    }
  });
});

const EmployeeSchedule = getOrCreateModel<IEmployeeSchedule>("EmployeeSchedule", EmployeeScheduleSchema);
