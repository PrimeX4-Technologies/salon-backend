...
import { getOrCreateModel, isSafeExternalHttpsUrl } from "../core/shared.js";
...
const ExternalConnectorSchema = new Schema<IExternalConnector>(
  {
    ...
    scope: { type: String, enum: ["business", "branch"], default: "business" },
...
ExternalConnectorSchema.index({ provider: 1, status: 1 });
ExternalConnectorSchema.index({ branchId: 1, status: 1 });
...
ExternalConnectorSchema.pre("validate", function () {
  ...
  if (this.scope === "branch" && !this.branchId) {
    this.invalidate("branchId", "A branch-scoped connector requires branchId");
  }
  if (this.scope === "business" && this.branchId) {
