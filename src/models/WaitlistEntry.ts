...
WaitlistEntrySchema.index({ salonId: 1, locationId: 1, status: 1, windowStartAt: 1, priority: -1 });
WaitlistEntrySchema.index({ salonId: 1, customerId: 1, status: 1 });
...
WaitlistEntrySchema.pre("validate", function () {
  ...
  if (this.serviceIds.length === 0) {
    this.invalidate("serviceIds", "At least one service is required");
  }
  ...
  if (this.status === "offered" && !this.offeredUntil) {
    this.invalidate("offeredUntil", "An offered waitlist entry requires an offer expiry");
  }
